...
import { Bot, Mail, Shield, Cloud, Layout, FileText, Search, Plus, Pencil, Calendar, Database, Globe, Lock, MessageCircle, Monitor } from 'lucide-react';
...
function App() {
  ...
  // Fetch apps from Supabase on mount
  useEffect(() => {
    fetchApps();
  }, []);
  ...
  const fetchApps = async () => {
    try {
      ...
      if (data) {
        // Map Supabase data to AppData interface
        const mappedApps: AppData[] = data.map((item: GamaHubApp) => ({
          id: item.id!,
          title: item.nome,
          description: item.descricao,
          iconName: item.icone,
          statusColor: 'green', // Default as per requirements (not in DB)
          cardColor: item.cor,
          url: item.url_app
        }));
        setApps(mappedApps);
  ...
  return (
    <MainLayout>
      ...
      {/* Main Grid Layout - Adjusted for Wider Left Column (approx 40% wider) */}
      <div className="grid grid-cols-1 lg:grid-cols-[1.3fr_3fr] gap-8 min-h-[600px]">
        ...
        {/* Right Section: Apps Grid - Spans rest */}
        <div className="flex flex-col gap-8">
          ...
          {/* We use a sub-grid here for the 2x2 or 3x2 layout of cards */}
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 auto-rows-fr overflow-y-auto max-h-[65vh] p-2 pb-6 no-scrollbar">
            ...
            {!isLoading && apps.map((app) => {
              ...
              if (app.iconName === 'favicon' && app.url) {
                ...
                // CASE 2: Uploaded Custom Image (URL)
                iconNode = (
                  <img
                    src={app.iconName}
                    alt={`${app.title} icon`}
                ...
                iconNode = React.cloneElement(IconComponent as React.ReactElement<{ className?: string }>, { className: getIconClass(app.statusColor) });
              }
